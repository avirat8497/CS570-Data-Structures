...
public class LRUCache<K,T extends Cacheable<K>> {
    ...
    public void modifySize(int newSize) {
        ...
        while (it.hasNext()&& i < newSize ){
            K k = it.next();
            tempL.add(k);
            tempM.put(k,persister.getItem(k));
            i++;
        }
        lruList.clear();
        itemMap.clear();
    ...
    public T getItem(K key) {
        Cacheable item = persister.getItem(key);
    ...
    public void putItem(T item) {
        ...
        if (persister.getItem(item.getKey())==null)
        {
    ...
    public T removeItem(K key) {
        if(itemMap.containsKey(key)||lruList.contains(key)){
            lruList.remove(key);
            itemMap.remove(key);
            return (T) (persister.removeItem(key));
        }
        return null;
    }
    ...
    public double getFaultRatePercent() {
        return (counter==0? 0: (long) faults/counter *100);
    }
